...
partial class Build
{
    ...
    Target Hotfix => _ => _
        ...
        .Executes(() =>
        {
            var masterVersion = GitVersion(s => s
                ...
                .SetBranch(TrunkBranch)
                .EnableNoFetch()
                .DisableProcessLogOutput()).Result;

            if (!GitRepository.IsOnHotfixBranch())
            {
                Checkout($"{HotfixBranchPrefix}/{masterVersion.Major}.{masterVersion.Minor}.{masterVersion.Patch + 1}", start: TrunkBranch);
            }
            else
            {
                FinishReleaseOrHotfix();
    ...
    void FinishReleaseOrHotfix()
    {
        ...
        Git($"push origin {TrunkBranch} {DevelopBranch} {MajorMinorPatchVersion}");
    }
